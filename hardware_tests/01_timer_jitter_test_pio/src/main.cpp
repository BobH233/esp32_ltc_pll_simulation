...
// ==================== 快速GPIO操作宏 ====================
// 注意：在 IRAM 中断中使用标准 API，避免直接访问寄存器
...
static void print_psram_info() {
    ...
    if (psram_size == 0) {
        ESP_LOGW(TAG, "⚠️  未检测到 PSRAM！");
        ESP_LOGW(TAG, "请检查:");
        ESP_LOGW(TAG, "  1. sdkconfig 中是否启用 CONFIG_SPIRAM=y");
        ESP_LOGW(TAG, "  2. 芯片型号是否包含 'R' (如 N16R8)");
        ESP_LOGW(TAG, "  3. platformio.ini 中的 psram_type 配置");
    } else {
        ESP_LOGI(TAG, "✅ PSRAM 已启用");
        ESP_LOGI(TAG, "   总容量: %zu MB", psram_size / (1024 * 1024));
        ESP_LOGI(TAG, "   可用内存: %zu KB",
                 heap_caps_get_free_size(MALLOC_CAP_SPIRAM) / 1024);

#ifdef CONFIG_SPIRAM_MODE_OCT
        ESP_LOGI(TAG, "   模式: OPI (快速)");
#else
        ESP_LOGI(TAG, "   模式: Quad (标准)");
#endif
    }
}
...
static void IRAM_ATTR timer_callback(void* arg) {
    ...
    toggle_state ^= 1;
    gpio_set_level(TEST_OUTPUT_GPIO, toggle_state);

    // 3. 记录时间戳到数组（仅在数据采集阶段）
    if (test_running && sample_index < SAMPLE_COUNT) {
        sample_timestamps[sample_index] = timestamp;

        size_t idx = sample_index;
        idx++;
        sample_index = idx;
...
extern "C" void app_main(void) {
    ESP_LOGI(TAG, "");
    ESP_LOGI(TAG, "╔══════════════════════════════════════════════════════╗");
    ESP_LOGI(TAG, "║   ESP32-S3 LTC Timer Jitter Test                    ║");
    ESP_LOGI(TAG, "║   定时器精度验证 - PlatformIO 版本                   ║");
    ESP_LOGI(TAG, "╚══════════════════════════════════════════════════════╝");
    ESP_LOGI(TAG, "");
    ...
    gpio_set_level(TEST_OUTPUT_GPIO, 0);
    gpio_set_level(TEST_STATUS_GPIO, 0);
    ...
    if (psram_size > 0) {
        // 有 PSRAM，优先从 PSRAM 分配
        sample_timestamps = (int64_t*)heap_caps_malloc(
            SAMPLE_COUNT * sizeof(int64_t),
            MALLOC_CAP_SPIRAM
        );
        jitter_data = (int32_t*)heap_caps_malloc(
            (SAMPLE_COUNT - WARMUP_SAMPLES) * sizeof(int32_t),
            MALLOC_CAP_SPIRAM
        );

        if (sample_timestamps && jitter_data) {
            ESP_LOGI(TAG, "✅ 已从 PSRAM 分配 %zu KB",
                     (SAMPLE_COUNT * sizeof(int64_t) +
                      (SAMPLE_COUNT - WARMUP_SAMPLES) * sizeof(int32_t)) / 1024);
        }
    } else {
        // 无 PSRAM，从普通堆分配
        ESP_LOGW(TAG, "未检测到 PSRAM，使用内部 RAM（可能内存不足）");
    ...
    if (!sample_timestamps || !jitter_data) {
        ESP_LOGE(TAG, "❌ 内存分配失败！");
        ESP_LOGE(TAG, "解决方法：");
        ESP_LOGE(TAG, "  1. 检查 PSRAM 是否正确配置");
        ESP_LOGE(TAG, "  2. 减小 SAMPLE_COUNT (当前: %d)", SAMPLE_COUNT);
        return;
    }
    ...
    ESP_LOGI(TAG, "");
    ESP_LOGI(TAG, "硬件连接:");
    ESP_LOGI(TAG, "  GPIO %d → 示波器/逻辑分析仪（测试输出）", TEST_OUTPUT_GPIO);
    ESP_LOGI(TAG, "  GPIO %d → LED/示波器（状态指示：高=测试中）", TEST_STATUS_GPIO);
    ESP_LOGI(TAG, "");
    ESP_LOGI(TAG, "3秒后开始...");
    ...
    // 【调试】先检查原始数据
    ESP_LOGI(TAG, "【调试】检查原始时间戳数据...");
    ESP_LOGI(TAG, "  前10个时间戳:");
    for (int i = 0; i < 10 && i < SAMPLE_COUNT; i++) {
        ESP_LOGI(TAG, "    [%d] = %" PRId64 " μs", i, sample_timestamps[i]);
    }
    ESP_LOGI(TAG, "  后10个时间戳:");
    for (int i = SAMPLE_COUNT - 10; i < SAMPLE_COUNT; i++) {
        ESP_LOGI(TAG, "    [%d] = %" PRId64 " μs", i, sample_timestamps[i]);
    }

    // 计算前几个周期
    ESP_LOGI(TAG, "  前20个周期:");
    for (int i = 1; i < 21 && i < SAMPLE_COUNT; i++) {
        int64_t period = sample_timestamps[i] - sample_timestamps[i-1];
        ESP_LOGI(TAG, "    周期[%d] = %" PRId64 " μs", i, period);
    }

    // 统计周期分布
    int period_99 = 0, period_100 = 0, period_101 = 0, period_other = 0;
    for (size_t i = 1; i < SAMPLE_COUNT; i++) {
        int64_t period = sample_timestamps[i] - sample_timestamps[i-1];
        if (period == 99) period_99++;
        else if (period == 100) period_100++;
        else if (period == 101) period_101++;
        else period_other++;
    }
    ESP_LOGI(TAG, "  周期分布统计:");
    ESP_LOGI(TAG, "    99 μs:  %d 次 (%.2f%%)", period_99, period_99 * 100.0 / SAMPLE_COUNT);
    ESP_LOGI(TAG, "    100 μs: %d 次 (%.2f%%)", period_100, period_100 * 100.0 / SAMPLE_COUNT);
    ESP_LOGI(TAG, "    101 μs: %d 次 (%.2f%%)", period_101, period_101 * 100.0 / SAMPLE_COUNT);
    ESP_LOGI(TAG, "    其他:   %d 次 (%.2f%%)", period_other, period_other * 100.0 / SAMPLE_COUNT);
    ...
    ESP_LOGI(TAG, "");
    ESP_LOGI(TAG, "╔══════════════════════════════════════════════════════╗");
    ESP_LOGI(TAG, "║   所有测试完成！系统保持空闲状态                     ║");
    ESP_LOGI(TAG, "╚══════════════════════════════════════════════════════╝");
    ESP_LOGI(TAG, "");
    ESP_LOGI(TAG, "💡 提示：可以用示波器测量 GPIO %d 的波形质量", TEST_OUTPUT_GPIO);
}
